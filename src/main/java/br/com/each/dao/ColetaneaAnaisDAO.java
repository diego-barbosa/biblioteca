...
@Component
public class ColetaneaAnaisDAO {

	private Connection connection;

	private PreparedStatement pstm;

	public ColetaneaAnaisDAO() {
		try {
	...
	public void salva(ColetaneaAnais coletaneaPeriodico) {
		try {
			this.connection = ConnectionFactory.getConnection();
			...
			while (set.next()) {
				...
				for (int i = 0; i < teste.length; i++) {
					...
					pstm = this.connection
							.prepareStatement("insert into coletanea_publicacao (id_artigo_anais_de_conferencias,cod_coletanea) values (?,?)");
