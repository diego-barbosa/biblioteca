...
@Component
public class ColetaneaLivroDAO {

	private Connection connection;

	private PreparedStatement pstm;

	public ColetaneaLivroDAO() {
		try {
	...
	public void salva(ColetaneaLivro coletaneaLivro) {
		try {
			this.connection = ConnectionFactory.getConnection();

			pstm = this.connection
					.prepareStatement("insert into coletanea (nome_coletanea) values (?)");
			pstm.setString(1, coletaneaLivro.getNomeColetanea());
			pstm.executeUpdate();
			pstm.close();
			...
			while (set.next()) {
				...
				String[] teste = coletaneaLivro.getIdPublicacoes().split(",");
				for (int i = 0; i < teste.length; i++) {

					pstm = this.connection
							.prepareStatement("insert into coletanea_publicacao (id_artigo_livro,cod_coletanea) values (?,?)");
